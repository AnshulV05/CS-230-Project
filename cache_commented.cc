...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu])
    {
        ...
        /** IF policy is Inclusive we need to invalidate victim addressed block from above caches **/
        if (do_fill && inclusiveness == INCLUSIVE)
        {
            // If current cache is L2C just need to invalidate in L1D and L1I
            if (cache_type == IS_L2C)
            {
                BLOCK *b = upper_level_dcache[fill_cpu]->invalidate_entry(block[set][way].address);
                // Check if block is dirty. If dirty then add to LLC
                do_fill = my_add_wq(b, set, way, do_fill, mshr_index, fill_cpu);
                if (!b)
                {
                    b = upper_level_icache[fill_cpu]->invalidate_entry(block[set][way].address);
                    do_fill = my_add_wq(b, set, way, do_fill, mshr_index, fill_cpu);
                }
            }
            // If current cache is L2C just need to invalidate in L1D, L1I and L2C
            else if (cache_type == IS_LLC)
            {
                ...
                if (b && do_fill)
                {
                    b = upper_level_dcache[fill_cpu]->upper_level_dcache[fill_cpu]->invalidate_entry(block[set][way].address);
                    do_fill = my_add_wq(b, set, way, do_fill, mshr_index, fill_cpu);

                    if (!b)
                    {
                        b = upper_level_dcache[fill_cpu]->upper_level_icache[fill_cpu]->invalidate_entry(block[set][way].address);
                        do_fill = my_add_wq(b, set, way, do_fill, mshr_index, fill_cpu);
                    }
                }
            }
        ...
        if (do_fill)
        {
            ...
            /** Fill entry into current cache only if Policy is not exculsive or cache is L1 **/
        ...
        if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) && (RQ.occupancy > 0))
        {
            ...
            { // read hit
                ...
                if (RQ.entry[index].fill_level < fill_level)
                {
                    ...
                    /** If policy is exclusive then remove current block which is sent upwards **/
                    if (inclusiveness == EXCLUSIVE && (cache_type == IS_LLC || cache_type == IS_L2C))
                    {
...
BLOCK *CACHE::invalidate_entry(uint64_t inval_addr)
{
...
/** Checks if block is dirty and adds it to lower level writeback queue if its dirty **/
int CACHE::my_add_wq(BLOCK *b, int set, int way, int do_fill, int mshr_index, int fill_cpu)
{
    if (b && b->dirty)
    {
        if (lower_level->get_occupancy(2, b->address) == lower_level->get_size(2, b->address))
        {
            // lower level WQ is full, cannot replace this victim
            do_fill = 0;
            lower_level->increment_WQ_FULL(b->address);
            STALL[MSHR.entry[mshr_index].type]++;
        }
        else
        {
            ...
            writeback_packet.fill_level = fill_level << 1;
            writeback_packet.cpu = fill_cpu;
            writeback_packet.address = b->address;
            writeback_packet.full_addr = b->full_addr;
            writeback_packet.data = b->data;
            writeback_packet.instr_id = MSHR.entry[mshr_index].instr_id;
            writeback_packet.ip = 0; // writeback does not have ip
            writeback_packet.type = WRITEBACK;
            writeback_packet.event_cycle = current_core_cycle[fill_cpu];
