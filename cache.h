...
class CACHE : public MEMORY {
  public:
    ...
    void add_mshr(PACKET *packet),
         ...
         seg_update(uint32_t set, uint32_t way,uint8_t hit),
    ...
    uint32_t get_set(uint64_t address),
             ...
             seg_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type);
};
